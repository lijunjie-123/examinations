...
/******������������Flex*******/
function InitFlexPdfViewer(divid){//����������
...
videoMain.push('id="FlexPdfViewer" width="'+_divWidth+'" height="'+_divHeigth+'"');
videoMain.push('codebase="http://fpdownload.macromedia.com/get/flashplayer/current/swflash.cab">');
videoMain.push('<param name="movie" value="Js/control/FlexPaperViewer.swf" />');
...
videoMain.push('<embed src="Js/control/FlexPaperViewer.swf" quality="high" bgcolor="#869ca7"');
videoMain.push('width="'+_divWidth+'" height="'+_divHeigth+'" name="FlexPdfViewer" align="middle"');
...
function SetFlexPdfViewerPath(path){
	setTimeout(function(){
		var flexPdfViewer = document.getElementById('FlexPdfViewer');
		try{
			if(flexPdfViewer.SetPdfPath(path)){
				return;
			}
		}catch(e){
			SetFlexPdfViewerPath(path);
		}
		},10);
}
